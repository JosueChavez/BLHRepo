...
public class Operaciones {
        ...
        try {
            ...
            if (resu.next() == true) {
                ...
                for (int i = 0; i < trayectos.getRowCount(); i++) {
                    PreparedStatement sen = con.conectar().prepareStatement("INSERT INTO VisitaDonacion (Fecha,Hora,Kilometraje,LugarSalida,LugarLlegada,Distancia,Galones,JVPM,IdUnidad) VALUES ('" + visita.getFecha() + "','" + trayectos.getValueAt(i, 0) + "','" + Double.parseDouble(trayectos.getValueAt(i, 1).toString()) + "','" + trayectos.getValueAt(i, 2) + "','" + trayectos.getValueAt(i, 3) + "','" + Double.parseDouble(trayectos.getValueAt(i, 4).toString()) + "','" + "" + "','" + visita.getJvpm() + "','" + idUnidad + "')");
                    sen.executeUpdate();
                }
    ...
    public void listarRutas(JTable table) {
        try {
            ...
            ResultSet r = s.executeQuery("SELECT IdVisitaDonacion, Fecha, Hora, LugarSalida, LugarLlegada, Distancia, JVPM FROM VisitaDonacion ORDER BY IdVisitaDonacion asc");
    ...
    public void BuscarRutas(JTable table, String campo, String value) {
        try {
            ...
            int dato = 1;
            Statement s = con.conectar().createStatement();
            ResultSet r = s.executeQuery("SELECT * FROM VisitaDonacion WHERE " + campo + " LIKE '" + value + "%' ORDER BY IdVisitaDonacion asc");
