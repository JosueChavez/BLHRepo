...
//        this.setLocationRelativeTo(null);
    ...
    private void initComponents() {
        ...
        jtDonaciones.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "# de Frasco", "Fecha Donación", "Ración Aceptada", "Ración Descartada", "Fecha de Vencimiento", "Tipo de Donación"
            }
        ));
...
private void jbAbrirDonacionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbAbrirDonacionActionPerformed
    ...
    try {
        ...
        NuevaDonacionModf.jcFechaDonacion.setDate(enviarFecha(jtDonaciones.getValueAt(jtDonaciones.getSelectedRow(), 1).toString()));
        NuevaDonacionModf.jcFechaVenc.setDate(enviarFecha(jtDonaciones.getValueAt(jtDonaciones.getSelectedRow(), 4).toString()));
        String fec = jtDonaciones.getValueAt(jtDonaciones.getSelectedRow(), 1).toString();
        if (jtDonaciones.getValueAt(jtDonaciones.getSelectedRow(), 5).toString().equals("Local")) {
            NuevaDonacionModf.jrbLocal.setSelected(true);
            tip = "Local";
        }
        if ("Domiciliar".equals(jtDonaciones.getValueAt(jtDonaciones.getSelectedRow(), 5))) {
            NuevaDonacionModf.jrbDomiciliar.setSelected(true);
            tip = "Domiciliar";
        }
        NuevaDonacionModf.jtfFrasco.setText(jtDonaciones.getValueAt(jtDonaciones.getSelectedRow(), 0).toString());
        ...
        Donacion don = new Donacion(fec, tip, racA, racD, d, jtDonaciones.getValueAt(jtDonaciones.getSelectedRow(), 4).toString(), jtDonaciones.getValueAt(jtDonaciones.getSelectedRow(), 0).toString(), "", "");
        ...
        if (jtDonaciones.getSelectedRow() == -1) {
            ...
            if (msj == 0) {
                if (opU.isAdmin()) {
                    ...
                    opD.eliminarDonacion(dc, jtDonaciones.getValueAt(jtDonaciones.getSelectedRow(), 1).toString(), jtDonaciones.getValueAt(jtDonaciones.getSelectedRow(), 2).toString());
//                    usu = opU.usuarioUp();
//                    String par = "Elm don fecha: " + jtDonaciones.getValueAt(jtDonaciones.getSelectedRow(), 1) + "dontCarnet: " + dc;
//                    opU.setEstado(usu, par);
                    ...
                    JOptionPane.showMessageDialog(null, "Usted no esta autorizado para realizar esta acción\nsolo un administrador puede hacerlo", "Error", JOptionPane.ERROR_MESSAGE);
                }
