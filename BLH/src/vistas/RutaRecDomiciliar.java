...
private void jbAnyadirMedicoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbAnyadirMedicoActionPerformed
        ...
        try {
            ...
            switch (var) {
                case 1:
                    ...
                    break;
                case 2:
                    RecoleccionDomiciliar.indiceOculto = jlIndiceOc.getText();
                    new InstanceOfTransport().transporte.setVisible(true);
                    RecoleccionDomiciliar.jlIndiceOculto.setText(ind);
                    RecoleccionDomiciliar.jbAnyadir.setEnabled(false);
                    motorista = "";
    ...
    private void jbGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbGuardarActionPerformed
        try {
            if (jtTrayectos.getRowCount() == 0) {
                JOptionPane.showMessageDialog(rootPane, "¡Incremente las Filas!");
            } else {
                ...
                for (int i = 0; i < jtTrayectos.getRowCount(); i++) {
                    for (int j = 0; j < jtTrayectos.getColumnCount(); j++) {
                        if (jtTrayectos.getValueAt(i, j).equals("")) {
                            completo = false;
                        } else {
                            completo = true;
                        }
                ...
                for (int j = 0; j < jtTrayectos.getRowCount(); j++) {
                    try {
                        dato = Double.parseDouble(jtTrayectos.getValueAt(j, 1).toString());
                        dato1 = Double.parseDouble(jtTrayectos.getValueAt(j, 4).toString());
                        correcto = true;
                    } catch (Exception e) {
                        JOptionPane.showMessageDialog(rootPane, "Error al ingresar el kilometraje y/o distancia  " + e.getMessage() + "\n Fila: " + (j + 1) + "");
                        correcto = false;
                        break;
                    }
                }
                if (jtTrayectos.getRowCount() > 1 && completo == true && correcto == true) {
                    if (!jtfMedico.getText().equals("") && !jtfMotorista.getText().equals("") && !jtfVehiculo.getText().equals("")) {
                        op.almacenarRuta(nuevaVisita(), trayectos);
                        limpiarGUI();
                        comoSalir();
                    } else {
                        JOptionPane.showMessageDialog(rootPane, "Revise los campos: Médico, Motorista y Vehículo");
                    }
                } else {
...
private void jtTrayectosFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_jtTrayectosFocusLost
    ...
    private void jSpinner1StateChanged(javax.swing.event.ChangeEvent evt) {//GEN-FIRST:event_jSpinner1StateChanged
        ...
        try {
            ...
            valor = Integer.parseInt(jSpinner1.getValue().toString());
            rows = jtTrayectos.getRowCount() + 1;
            DefaultTableModel model = (DefaultTableModel) jtTrayectos.getModel();
            if (jSpinner1.getModel().getNextValue().equals(valor + 1) && valor == rows) {
                Object fila[] = {"", "", "", "", ""};
                model.addRow(fila);
            } else {
                model.removeRow(jtTrayectos.getRowCount() - 1);
            }
